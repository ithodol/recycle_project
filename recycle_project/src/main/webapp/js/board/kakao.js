    ...
    for ( var i=0; i<places.length; i++ ) {
        ...
        (function(marker, title) {
			...
			kakao.maps.event.addListener(marker, 'click', function() {
			    ...
			    var position = marker.getPosition(); // 마커의 위치 가져오기
			    panTo(position.getLat(), position.getLng()); // 해당 위치로 지도 이동
...
function displayInfowindow(marker, title) {
    var content = 	`
	        <div style="padding:10px; width:200px; text-align:center;">
	            ...
	            <button onclick="confirmPlace('${title}')">확인</button>
...
function panTo(lat, lon) {
    ...
    map.panTo(moveLatLon); // 해당 위치 부드럽게 이동     
}
...
// 확인 버튼 기능 (장소명 콘솔 출력)
function confirmPlace(title) {
    console.log("선택한 장소:", title);
    alert("선택한 장소: " + title);
	infowindow.close();
